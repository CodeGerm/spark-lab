...
object RunKMeans {
  ...
  def main(args: Array[String]): Unit = {
    val sc = new SparkContext(new SparkConf().setAppName("K-means").setMaster("local[2]"))
    val rawData = sc.textFile("data/kddcup.data_0.1_percent")
    /*
     * This take aggregates and prints all labels of
     * raw data. Cluster raw data into 2 
     * clusters and shows node 
    ...
    /*
     * This take shows the result of clustering 
     * raw data into 5 to 100 clusters
     */
    println("take1: ")
    clusteringTake1(rawData)

    /*
     * This take normalizes all the features
     * and shows the result of clustering 
     * pre-processed data into 60 to 120 clusters
     */
    println("take2: ")
    clusteringTake2(rawData)

    /*
     * This take encodes the categorical
     * features, normalizes all the features
     * and shows the result of clustering 
     * pre-processed data into 60 to 120 clusters
     */
    println("take3: ")
    ...
    /*
     * This take does the same thing as 
     * take 3 and shows the result by 
     * using entropy based scoring
     */
    println("take4: ")
    clusteringTake4(rawData)

    /*
     * This take uses the best k value 
     * according to result in take4
     * and find the data further than 
     * top 100 as anomolies
     */
    println("anomalies: ")
    anomalies(rawData)
  }
  ...
  def visualizationInR(rawData: RDD[String]): Unit = {
    ...
    val sample = data.map(datum =>
      model.predict(datum) + "," + datum.toArray.mkString(",")).sample(false, 0.05)

    sample.saveAsTextFile("data/output/sample")
